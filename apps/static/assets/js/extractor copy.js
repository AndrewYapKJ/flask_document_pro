...
class ExtractorManager {
    ...
    async loadAndRenderPDF(fileURL, container) {
        try {
            ...
            for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
                const pageHeight = await this.renderPDFPage(pdf, pageNum, pagesContainer, baseDisplayScale);
                totalDisplayHeight += pageHeight;
            }
            // Ensure parent container can scroll to the bottom
            pagesContainer.style.minHeight = totalDisplayHeight + 'px';
    ...
    async renderPDFPage(pdf, pageNum, container, displayScale = null) {
        try {
            ...
            // Return displayHeight for parent container sizing
